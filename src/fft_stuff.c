...
void setup(fft_data *data, int N) {
	...
	data->N = N;
	data->data_in = (fftw_complex *) fftw_malloc(sizeof(fftw_complex) * N);
	data->data_out = (fftw_complex *) fftw_malloc(sizeof(fftw_complex) * N);
	...
	data->plan_forward = fftw_plan_dft_1d(N, data->data_in, data->data_out, -1, FFTW_MEASURE);
	data->plan_backward = fftw_plan_dft_1d(N, data->data_out, data->data_in, 1, FFTW_MEASURE);
